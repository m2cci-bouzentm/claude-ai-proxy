...
import { readClaudeCredentials, deleteClaudeCredentials } from "./keychain";
...
function seedFromClaude(): storage.OAuthEntry {
  ...
  if (deleteClaudeCredentials()) {
    console.log("[auth] deleted Claude Code credentials — run `claude` and /login for independent CLI token");
  }
...
export async function getAuth(): Promise<AuthResult> {
  ...
  const needsRefresh = !currentAuth.access || currentAuth.expires < Date.now() + BUFFER_MS;
  if (!needsRefresh) {
    return {
  ...
  refreshPromise ??= refreshAccessToken(currentAuth.refresh)
    .then((tokens) => {
      ...
      console.log(`[auth] refreshed, expires ${new Date(currentAuth.expires).toISOString()}`);
    })
    .catch((err) => {

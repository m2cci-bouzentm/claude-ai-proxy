...
const SYSTEM_PROMPT = JSON.parse(fs.readFileSync(SYSTEM_PROMPT_PATH, "utf-8"));
...
function parseSSE(body: string): { content: string; usage: { input_tokens: number; output_tokens: number } } {
  let content = "";
  ...
  for (const line of body.split("\n")) {
    ...
    try {
      ...
      if (evt.type === "message_start") usage.input_tokens = evt.message?.usage?.input_tokens || 0;
      if (evt.type === "message_delta") usage.output_tokens = evt.usage?.output_tokens || 0;
    } catch {}
  }
  return { content, usage };
}

async function callAPI(messages: Message[], model: string, maxTokens: number): Promise<Response> {
...
export async function createToolResponse(
  ...
  const system = (Array.isArray(SYSTEM_PROMPT) ? SYSTEM_PROMPT : [{ type: "text", text: SYSTEM_PROMPT }])
    .map((block: { type: string; text?: string }) => block.type === "text" && block.text?.startsWith("x-anthropic-billing-header:")
      ? { ...block, text: block.text.replace(/cc_version=\d+\.\d+\.\d+/, `cc_version=${toolClientVersion}`) }
      : block);
  if (clientSystem) system.push({ type: "text", text: clientSystem });
  return fetch(API_URL, {
    method: "POST",
    headers: { ...buildHeaders(auth), "User-Agent": `claude-cli/${toolClientVersion} (external, sdk-cli)` },
